...
const AdminPanel = ({ currentData, onUpdateLocalData }) => {
  ...
  const addRabbi = () => {
      ...
      setEditingItem({ id: newId, name: '', category: 'Tanna', generation: 1, description: '' });
      setView('edit-rabbi');
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="fixed inset-0 bg-slate-950/95 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="p-8 max-w-md w-full bg-slate-900 rounded-xl border border-slate-800 shadow-2xl relative">
                        ...
                        {data.relationships.map((rel, idx) => {
                             ...
                             return (
                                <div key={idx} className="flex justify-between items-center p-3 bg-slate-800/50 rounded border border-slate-700/50">
                                    ...
                                    }} className="text-red-500 hover:text-red-400 px-2"><Trash2 size={16}/></button>
                                </div>
                             )
                        })}
                    </div>
